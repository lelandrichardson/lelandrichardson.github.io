...
Making things generic adds complexity, but isn't important to understand how things work. Rather than go into details about how this generalism is achieved, let’s define a basic tree-like data structure that we can use for the purposes of this blog post.
...
Of course we have glossed over a lot of complexity that might go into the `renderNodeToScreen` method, but for the purposes of this article, its implementation is not important.
...
There's still a lot more to cover, but I think that might be enough for one blog. There are several things Compose is doing or planning to do that areis not covered in this blog:
